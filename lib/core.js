...
let cluster = require('cluster'), fs = require('fs'), _ = require('lodash'),
    ...
    server = null, parentPid = null, logger = null, funcMasterExit = null;
...
let core = {
  ...
  forEach: (collection, funcRow, funcEnd) => {
    let keys = _.keys(collection), funcEach = (cont) => {
      let key = keys.shift();
      if (key && (cont !== false)) {
        if (funcRow.length == 1) {
          let r = funcRow(collection[key]);
          setImmediate(funcEach, r);
...
// start method
let start = (services, end, onMasterExit) => {
  do {
    ...
    if (isMaster) { // master
      ...
      // overwrite function onExit
      if (typeof onMasterExit === 'function') {
        funcMasterExit = onMasterExit;
      }
...
module.exports = (options, services, end, onMasterExit) => {
  // load and config
  ...
  start(services, () => {
    if (typeof end === 'function') {
      end(core, server ? server.getServer() : null);
    }
  }, onMasterExit);
};
